---
title: '실전 함수형 프로그래밍'
description: '멀티패러다임 프로그래밍을 읽고 FxTS로 실무에 함수형 프로그래밍 적용하기. 복잡한 데이터 처리와 비동기 작업을 선언적으로 해결하는 예제들을 살펴보자.'
...
![type-inference.png](practical-functional-programming/type-inference.png)
...
2. `forEach`를 사용한 반복 실행
    - 이터러블을 (여기서는 무한 반복)을 순회하며 작업을 실행한다.
3. `Promise.all`로 동시 실행
...
실제 이터러블을 소비하여 최종 결과를 만들어낸다. 한 번 호출하면 지연이 해제되고 실제 순회가 일어난다.
