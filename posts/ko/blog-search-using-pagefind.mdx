...
```json
"scripts": {
  "postbuild": "npx pagefind --site dist/client --output-path public/pagefind",
},
```

- `--site dist/client`: TanStack Start의 클라이언트 사이드 빌드 결과물이 저장되는 경로다.
- `--output-path public/pagefind`: 생성된 인덱스 파일을 public 폴더 내에 저장하여 런타임에 접근할 수 있도록 한다.
